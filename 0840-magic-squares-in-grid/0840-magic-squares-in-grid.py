...
class Solution:
    def numMagicSquaresInside(self, grid: List[List[int]]) -> int:
        ...
        def check(r, c):
            counter = [False] * 16
            counter[grid[r][c]] = True
            counter[grid[r][c+1]] = True
            counter[grid[r][c+2]] = True

            counter[grid[r+1][c]] = True
            counter[grid[r+1][c+1]] = True
            counter[grid[r+1][c+2]] = True
                
            counter[grid[r+2][c]] = True
            counter[grid[r+2][c+1]] = True
            counter[grid[r+2][c+2]] = True

            if sum(counter[1:10]) == 9:
                return True
            return False
